...
class AnalysisForm(forms.ModelForm):
     class Meta:
        model = Analysis
        fields = ['sentiment_score', 'facial_expression', 'emotion_intelligence', 'feedback']
        widgets = {
            'sentiment_score': forms.HiddenInput(),
            'facial_expression': forms.HiddenInput(),
            'emotion_intelligence': forms.HiddenInput(),
            'feedback': forms.Textarea(attrs={'class': 'form-control', 'rows': 5})
        }
        
        def save(self, commit=True):
            analysis = super().save(commit=False)
            if analysis.sentiment_score >= 0.7 and analysis.facial_expression == "confident" and analysis.emotion_intelligence == "high":
                analysis.feedback = "Great job! You came across as confident and knowledgeable in your responses. Keep up the good work."
            elif analysis.sentiment_score < 0.7 and analysis.facial_expression == "neutral" and analysis.emotion_intelligence == "low":
