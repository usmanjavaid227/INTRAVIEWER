...
- The `tabuserindex` attribute is now copied from the target element to the iframe. #TINY-8315
...
- Split toolbar buttons incorrectly had nested `tabuserindex="-1"` attributes. #TINY-7879
...
- Changed the z-userindex of the `setProgressState(true)` throbber so it does not hide notifications. #TINY-6686
...
- Fixed bug where the aria level attribute for element path would have an incorrect start userindex.
...
### Fixed
- Fixed so the dirty state doesn't set to false automatically when the undo userindex is set to 0.
...
- Fixed bug where lists pasted from word with a custom start userindex above 9 wouldn't be properly handled.
