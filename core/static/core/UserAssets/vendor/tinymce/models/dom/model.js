...
(function () {
    ...
    const nativeuserindexOf = Array.prototype.userindexOf;
    const nativePush = Array.prototype.push;
    const rawuserindexOf = (ts, t) => nativeuserindexOf.call(ts, t);
    const contains$2 = (xs, x) => rawuserindexOf(xs, x) > -1;
    ...
    const child$2 = (element, userindex) => {
      const cs = element.dom.childNodes;
      return Optional.from(cs[userindex]).map(SugarElement.fromDom);
    };
    ...
    const appendAt = (parent, element, userindex) => {
      child$2(parent, userindex).fold(() => {
        append$1(parent, element);
      }, v => {
        before$3(v, element);
      });
    ...
    const contains = (str, substr, start = 0, end) => {
      const idx = str.userindexOf(substr, start);
      if (idx !== -1) {
    ...
    const addCells = (gridRow, userindex, cells) => {
      const existingCells = gridRow.cells;
      const before = existingCells.slice(0, userindex);
      const after = existingCells.slice(userindex);
      const newCells = before.concat(cells).concat(after);
      return setCells(gridRow, newCells);
    };
    const addCell = (gridRow, userindex, cell) => addCells(gridRow, userindex, [cell]);
    const mutateCell = (gridRow, userindex, cell) => {
      const cells = gridRow.cells;
      cells[userindex] = cell;
    };
    ...
    const getCell = (gridRow, userindex) => gridRow.cells[userindex];
    const getCellElement = (gridRow, userindex) => getCell(gridRow, userindex).element;
    ...
    const generateColumns = rowData => {
      ...
      each$2(rowData.cells, column => {
        ...
        range$1(colspan, columnuserindex => {
          const coluserindex = userindex + columnuserindex;
          columnsGroup[coluserindex] = columnext(column.element, colspan, coluserindex);
        });
        userindex += colspan;
      });
      return columnsGroup;
    ...
    const getColumnAt = (warehouse, columnuserindex) => Optional.from(warehouse.columns[columnuserindex]);
    const Warehouse = {
      fromTable,
      generate: generate$1,
      getAt,
    ...
    const deduce = (xs, userindex) => {
      if (userindex < 0 || userindex >= xs.length - 1) {
        return Optional.none();
      }
      const current = xs[userindex].fold(() => {
        const rest = reverse(xs.slice(0, userindex));
      ...
      const next = xs[userindex + 1].fold(() => {
        const rest = xs.slice(userindex + 1);
        return findMap(rest, (a, i) => a.map(aa => ({
          value: aa,
          delta: i + 1
        })));
    ...
    const getLeftEdge = (userindex, cell) => {
      return colInfo(userindex, ltrEdge(cell));
    };
    const getRightEdge = (userindex, cell) => {
      return colInfo(userindex, rtlEdge(cell));
    };
    const getTop$1 = cell => {
      return absolute(cell).top;
    };
    const getTopEdge = (userindex, cell) => {
      return rowInfo(userindex, getTop$1(cell));
    };
    const getBottomEdge = (userindex, cell) => {
      return rowInfo(userindex, getTop$1(cell) + getOuter$1(cell));
    };
    const findPositions = (getInnerEdge, getOuterEdge, array) => {
      if (array.length === 0) {
        return [];
      }
      const lines = map$1(array.slice(1), (cellOption, userindex) => {
        return cellOption.map(cell => {
          return getInnerEdge(userindex, cell);
        });
      });
    ...
    const getDimension = (cellOpt, userindex, backups, filter, getter, fallback) => cellOpt.filter(filter).fold(() => fallback(deduce(backups, userindex)), cell => getter(cell));
    ...
    const makeCell = (list, seenSelected, rowuserindex) => {
      const row = list[rowuserindex].element;
    ...
    const isTextNodeWithCursorPosition = el => getOption(el).filter(text => text.trim().length !== 0 || text.userindexOf(nbsp) > -1).isSome();
    ...
    const ancestors$2 = (universe, start, end, isRoot = never) => {
      ...
      const prune = path => {
        const userindex = finduserindex(path, isRoot);
        return userindex.fold(() => {
          return path;
        }, ind => {
    ...
    const surround = (sizes, startuserindex, enduserindex, results, f) => f(sizes.slice(0, startuserindex)).concat(results).concat(f(sizes.slice(enduserindex)));
    const clampDeltaHelper = predicate => (sizes, userindex, delta, minCellSize) => {
      if (!predicate(delta)) {
        return delta;
      } else {
        const newSize = Math.max(minCellSize, sizes[userindex] - Math.abs(delta));
        const diff = Math.abs(newSize - sizes[userindex]);
    ...
    const resizeTable = () => {
      const calcFixedDeltas = (sizes, userindex, next, delta, minCellSize) => {
        const clampedDelta = clampNegativeDelta(sizes, userindex, delta, minCellSize);
        return surround(sizes, userindex, next + 1, [
          clampedDelta,
          0
        ], zero);
      };
      const calcRelativeDeltas = (sizes, userindex, delta, minCellSize) => {
        const ratio = (100 + delta) / 100;
        const newThis = Math.max(minCellSize, (sizes[userindex] + delta) / ratio);
        return map$1(sizes, (size, idx) => {
          const newSize = idx === userindex ? newThis : size / ratio;
          return newSize - size;
        });
      };
      const calcLeftEdgeDeltas = (sizes, userindex, next, delta, minCellSize, isRelative) => {
        if (isRelative) {
          return calcRelativeDeltas(sizes, userindex, delta, minCellSize);
        } else {
          return calcFixedDeltas(sizes, userindex, next, delta, minCellSize);
        }
      };
      const calcMiddleDeltas = (sizes, _prev, userindex, next, delta, minCellSize, isRelative) => calcLeftEdgeDeltas(sizes, userindex, next, delta, minCellSize, isRelative);
      const resizeTable = (resizer, delta) => resizer(delta);
      const calcRightEdgeDeltas = (sizes, _prev, userindex, delta, minCellSize, isRelative) => {
        if (isRelative) {
          return calcRelativeDeltas(sizes, userindex, delta, minCellSize);
        } else {
          const clampedDelta = clampNegativeDelta(sizes, userindex, delta, minCellSize);
          return zero(sizes.slice(0, userindex)).concat([clampedDelta]);
        }
      };
    ...
    const preserveTable = () => {
      const calcLeftEdgeDeltas = (sizes, userindex, next, delta, minCellSize) => {
        const idx = delta >= 0 ? next : userindex;
      ...
      const calcMiddleDeltas = (sizes, _prev, userindex, next, delta, minCellSize) => calcLeftEdgeDeltas(sizes, userindex, next, delta, minCellSize);
      const resizeTable = (resizer, delta, isLastColumn) => {
        if (isLastColumn) {
          resizer(delta);
        }
      };
      const calcRightEdgeDeltas = (sizes, _prev, _userindex, delta, _minCellSize, isRelative) => {
        if (isRelative) {
          return zero(sizes);
        } else {
      ...
      const clampTableDelta = (sizes, userindex, delta, minCellSize, isLastColumn) => {
        if (isLastColumn) {
          if (delta >= 0) {
            return delta;
          } else {
          ...
          return clampNegativeDelta(sizes, userindex, delta, minCellSize);
        }
      };
    ...
    const getColumn = (grid, userindex) => {
      return map$1(grid, row => {
        return getCell(row, userindex);
      });
    };
    const getRow = (grid, userindex) => {
      return grid[userindex];
    };
    const findDiff = (xs, comp) => {
      ...
      const first = xs[0];
      const userindex = finduserindex(xs, x => {
        return !comp(first.element, x.element);
      });
      return userindex.getOr(xs.length);
    };
    ...
    const toDetails = (grid, comparator) => {
      ...
      const updateSeen = (rowuserindex, columnuserindex, rowspan, colspan) => {
        for (let row = rowuserindex; row < rowuserindex + rowspan; row++) {
          for (let column = columnuserindex; column < columnuserindex + colspan; column++) {
            seen[row][column] = true;
          }
        }
      };
      return map$1(grid, (row, rowuserindex) => {
        const details = bind$2(row.cells, (cell, columnuserindex) => {
          if (seen[rowuserindex][columnuserindex] === false) {
            const result = subgrid(grid, rowuserindex, columnuserindex, comparator);
            updateSeen(rowuserindex, columnuserindex, result.rowspan, result.colspan);
    ...
    const toGrid = (warehouse, generators, isNew) => {
      ...
      each$2(warehouse.colgroups, colgroup => {
        const colgroupCols = [];
        for (let columnuserindex = 0; columnuserindex < warehouse.grid.columns; columnuserindex++) {
          const element = Warehouse.getColumnAt(warehouse, columnuserindex).map(column => elementnew(column.element, isNew, false)).getOrThunk(() => elementnew(generators.colGap(), true, false));
          colgroupCols.push(element);
        }
        grid.push(rowcells(colgroup.element, colgroupCols, 'colgroup', isNew));
      });
      for (let rowuserindex = 0; rowuserindex < warehouse.grid.rows; rowuserindex++) {
        const rowCells = [];
        for (let columnuserindex = 0; columnuserindex < warehouse.grid.columns; columnuserindex++) {
          const element = Warehouse.getAt(warehouse, rowuserindex, columnuserindex).map(item => elementnew(item.element, isNew, item.isLocked)).getOrThunk(() => elementnew(generators.gap(), true, false));
          rowCells.push(element);
        }
        const rowDetail = warehouse.all[rowuserindex];
    ...
    const splitCols = (grid, userindex, comparator, substitution) => {
      if (userindex > 0 && userindex < grid[0].cells.length) {
        each$2(grid, row => {
          const prevCell = row.cells[userindex - 1];
          let offset = 0;
          const substitute = substitution();
          while (row.cells.length > userindex + offset && comparator(prevCell.element, row.cells[userindex + offset].element)) {
            mutateCell(row, userindex + offset, elementnew(substitute, true, row.cells[userindex + offset].isLocked));
            offset++;
          }
        });
      }
    ...
    const splitRows = (grid, userindex, comparator, substitution) => {
      const rows = extractGridDetails(grid).rows;
      if (userindex > 0 && userindex < rows.length) {
        const rowPrevCells = rows[userindex - 1].cells;
        const cells = uniqueCells(rowPrevCells, comparator);
        each$2(cells, cell => {
          let replacement = Optional.none();
          for (let i = userindex; i < rows.length; i++) {
            for (let j = 0; j < cellLength(rows[0]); j++) {
              const row = rows[i];
    ...
    const colFill = (grid, amount, generators, startuserindex) => map$1(grid, row => {
      const newChildren = generateElements(amount, row, generators, never);
      return addCells(row, startuserindex, newChildren);
    });
    ...
    const mergeTables = (startAddress, gridA, gridBRows, generator, comparator, lockedColumns) => {
      ...
      for (let r = startRow; r < endRow; r++) {
        ...
        for (let c = startCol; c < endCol; c++) {
          ...
          const gridBColuserindex = c - startCol - skippedCol;
          const newCell = getCell(gridBRows[r - startRow], gridBColuserindex);
    ...
    const insertCols = (userindex, gridA, gridB, generator, comparator) => {
      splitCols(gridA, userindex, comparator, generator.cell);
      ...
      return map$1(fittedOldGrid, (gridRow, i) => {
        return addCells(gridRow, userindex, fittedNewGrid[i].cells);
      });
    };
    const insertRows = (userindex, gridA, gridB, generator, comparator) => {
      splitRows(gridA, userindex, comparator, generator.cell);
      ...
      return [
        ...oldCols,
        ...oldRows.slice(0, userindex),
        ...fittedNewGrid,
        ...oldRows.slice(userindex, oldRows.length)
      ];
    };

    const cloneRow = (row, cloneCell, comparator, substitution) => clone(row, elem => substitution(elem, comparator), cloneCell);
    const insertRowAt = (grid, userindex, example, comparator, substitution) => {
      const {rows, cols} = extractGridDetails(grid);
      const before = rows.slice(0, userindex);
      const after = rows.slice(userindex);
      const newRow = cloneRow(rows[example], (ex, c) => {
        const withinSpan = userindex > 0 && userindex < rows.length && comparator(getCellElement(rows[userindex - 1], c), getCellElement(rows[userindex], c));
        const ret = withinSpan ? getCell(rows[userindex], c) : elementnew(substitution(ex.element, comparator), true, ex.isLocked);
        return ret;
    ...
    const insertColumnAt = (grid, userindex, example, comparator, substitution) => map$1(grid, row => {
      const withinSpan = userindex > 0 && userindex < cellLength(row) && comparator(getCellElement(row, userindex - 1), getCellElement(row, userindex));
      const sub = getElementFor(row, userindex, row.section, withinSpan, example, comparator, substitution);
      return addCell(row, userindex, sub);
    });
    ...
    const notInStartRow = (grid, rowuserindex, coluserindex, comparator) => getCellElement(grid[rowuserindex], coluserindex) !== undefined && (rowuserindex > 0 && comparator(getCellElement(grid[rowuserindex - 1], coluserindex), getCellElement(grid[rowuserindex], coluserindex)));
    const notInStartColumn = (row, userindex, comparator) => userindex > 0 && comparator(getCellElement(row, userindex - 1), getCellElement(row, userindex));
    const isDuplicatedCell = (grid, rowuserindex, coluserindex, comparator) => notInStartRow(grid, rowuserindex, coluserindex, comparator) || notInStartColumn(grid[rowuserindex], coluserindex, comparator);
    const rowReplacerPredicate = (targetRow, columnHeaders) => {
      const entireTableIsHeader = forall(columnHeaders, identity) && isHeaderCells(targetRow.cells);
      return entireTableIsHeader ? always : (cell, _rowuserindex, coluserindex) => {
        const type = name(cell.element);
        return !(type === 'th' && columnHeaders[coluserindex]);
      };
    };
    const columnReplacePredicate = (targetColumn, rowHeaders) => {
      const entireTableIsHeader = forall(rowHeaders, identity) && isHeaderCells(targetColumn);
      return entireTableIsHeader ? always : (cell, rowuserindex, _coluserindex) => {
        const type = name(cell.element);
        return !(type === 'th' && rowHeaders[rowuserindex]);
      };
    };
    ...
    const rowScopeGenerator = (applyScope, columnHeaders) => (cell, rowuserindex, columnuserindex) => Optional.some(determineScope(applyScope, cell.element, 'col', columnHeaders[columnuserindex]));
    const columnScopeGenerator = (applyScope, rowHeaders) => (cell, rowuserindex) => Optional.some(determineScope(applyScope, cell.element, 'row', rowHeaders[rowuserindex]));
    ...
    const replaceIn = (grid, targets, comparator, substitute, replacer, genScope, shouldReplace) => {
      ...
      return map$1(grid, (row, rowuserindex) => {
        return mapCells(row, (cell, coluserindex) => {
          if (isTarget(cell)) {
            const newCell = shouldReplace(cell, rowuserindex, coluserindex) ? replacer(cell, comparator, substitute) : cell;
            genScope(newCell, rowuserindex, coluserindex).each(scope => {
    ...
    const getColumnCells = (rows, columnuserindex, comparator) => bind$2(rows, (row, i) => {
      return isDuplicatedCell(rows, i, columnuserindex, comparator) ? [] : [getCell(row, columnuserindex)];
    });
    const getRowCells = (rows, rowuserindex, comparator) => {
      const targetRow = rows[rowuserindex];
      return bind$2(targetRow.cells, (item, i) => {
        return isDuplicatedCell(rows, rowuserindex, i, comparator) ? [] : [item];
      });
    };
    const replaceColumns = (grid, userindexes, applyScope, comparator, substitution) => {
      const rows = extractGridDetails(grid).rows;
      const targets = bind$2(userindexes, userindex => getColumnCells(rows, userindex, comparator));
      const rowHeaders = map$1(rows, row => isHeaderCells(row.cells));
      const shouldReplaceCell = columnReplacePredicate(targets, rowHeaders);
      const scopeGenerator = columnScopeGenerator(applyScope, rowHeaders);
      return replaceIn(grid, targets, comparator, substitution, replace, scopeGenerator, shouldReplaceCell);
    };
    const replaceRows = (grid, userindexes, section, applyScope, comparator, substitution, tableSection) => {
      const {cols, rows} = extractGridDetails(grid);
      const targetRow = rows[userindexes[0]];
      const targets = bind$2(userindexes, userindex => getRowCells(rows, userindex, comparator));
      const columnHeaders = map$1(targetRow.cells, (_cell, userindex) => isHeaderCells(getColumnCells(rows, userindex, comparator)));
      const newRows = [...rows];
      each$2(userindexes, userindex => {
        newRows[userindex] = tableSection.transformRow(rows[userindex], section);
      });
      const newGrid = [
        ...cols,
        ...newRows
    ...
    const neighbours = (input, userindex) => {
      if (input.length === 0) {
      ...
      if (userindex === input.length - 1) {
        return ColumnContext.right(userindex - 1, userindex);
      }
      if (userindex > 0 && userindex < input.length - 1) {
        return ColumnContext.middle(userindex - 1, userindex, userindex + 1);
      }
      return ColumnContext.none();
    };
    const determine = (input, column, step, tableSize, resize) => {
      ...
      const onOnly = userindex => tableSize.singleColumnWidth(result[userindex], step);
      const onLeft = (userindex, next) => resize.calcLeftEdgeDeltas(result, userindex, next, step, tableSize.minCellWidth(), tableSize.isRelative);
      const onMiddle = (prev, userindex, next) => resize.calcMiddleDeltas(result, prev, userindex, next, step, tableSize.minCellWidth(), tableSize.isRelative);
      const onRight = (prev, userindex) => resize.calcRightEdgeDeltas(result, prev, userindex, step, tableSize.minCellWidth(), tableSize.isRelative);
    ...
    const recalculateWidthForColumns = (warehouse, widths) => {
      ...
      return map$1(groups, (column, userindex) => ({
        element: column.element,
        width: widths[userindex],
    ...
    const adjustWidth = (table, delta, userindex, resizing, tableSize) => {
      const warehouse = Warehouse.fromTable(table);
      const step = tableSize.getCellDelta(delta);
      const widths = tableSize.getWidths(warehouse, tableSize);
      const isLastColumn = userindex === warehouse.grid.columns - 1;
      const clampedStep = resizing.clampTableDelta(widths, userindex, step, tableSize.minCellWidth(), isLastColumn);
      const deltas = determine(widths, userindex, clampedStep, tableSize, resizing);
      const newWidths = map$1(deltas, (dx, i) => dx + widths[i]);
      recalculateAndApply(warehouse, newWidths, tableSize);
      resizing.resizeTable(tableSize.adjustTableWidth, clampedStep, isLastColumn);
    };
    const adjustHeight = (table, delta, userindex, direction) => {
      const warehouse = Warehouse.fromTable(table);
      const heights = getPixelHeights(warehouse, table, direction);
      const newHeights = map$1(heights, (dy, i) => userindex === i ? Math.max(delta + dy, minHeight()) : dy);
    ...
    const opInsertRowsBefore = (grid, details, comparator, genWrappers) => {
      const targetuserindex = details[0].row;
      const rows = uniqueRows(details);
      const newGrid = foldr(rows, (acc, row) => {
        const newG = insertRowAt(acc.grid, targetuserindex, row.row + acc.delta, comparator, genWrappers.getOrInit);
        return {
          grid: newG,
          delta: acc.delta + 1
        };
      ...
      return bundle(newGrid, targetuserindex, details[0].column);
    };
    const opInsertRowsAfter = (grid, details, comparator, genWrappers) => {
      const rows = uniqueRows(details);
      const target = rows[rows.length - 1];
      const targetuserindex = target.row + target.rowspan;
      const newGrid = foldr(rows, (newG, row) => {
        return insertRowAt(newG, targetuserindex, row.row, comparator, genWrappers.getOrInit);
      }, grid);
      return bundle(newGrid, targetuserindex, details[0].column);
    };
    const opInsertColumnsBefore = (grid, extractDetail, comparator, genWrappers) => {
      ...
      const targetuserindex = columns[0].column;
      const newGrid = foldr(columns, (acc, col) => {
        const newG = insertColumnAt(acc.grid, targetuserindex, col.column + acc.delta, comparator, genWrappers.getOrInit);
        return {
          grid: newG,
          delta: acc.delta + 1
        };
      ...
      return bundle(newGrid, details[0].row, targetuserindex);
    };
    const opInsertColumnsAfter = (grid, extractDetail, comparator, genWrappers) => {
      const details = extractDetail.details;
      const target = details[details.length - 1];
      const targetuserindex = target.column + target.colspan;
      const columns = uniqueColumns(details);
      const newGrid = foldr(columns, (newG, col) => {
        return insertColumnAt(newG, targetuserindex, col.column, comparator, genWrappers.getOrInit);
      }, grid);
      return bundle(newGrid, details[0].row, targetuserindex);
    };
    const opMakeColumnsHeader = (initialGrid, details, comparator, genWrappers) => {
      const columns = uniqueColumns(details);
      const columnuserindexes = map$1(columns, detail => detail.column);
      const newGrid = replaceColumns(initialGrid, columnuserindexes, true, comparator, genWrappers.replaceOrInit);
    ...
    const opUnmakeColumnsHeader = (initialGrid, details, comparator, genWrappers) => {
      const columns = uniqueColumns(details);
      const columnuserindexes = map$1(columns, detail => detail.column);
      const newGrid = replaceColumns(initialGrid, columnuserindexes, false, comparator, genWrappers.replaceOrInit);
      return bundle(newGrid, details[0].row, details[0].column);
    };
    ...
    const makeRowsSection = (section, applyScope) => (initialGrid, details, comparator, genWrappers, tableSection) => {
      const rows = uniqueRows(details);
      const rowuserindexes = map$1(rows, detail => detail.row);
      const newGrid = replaceRows(initialGrid, rowuserindexes, section, applyScope, comparator, genWrappers.replaceOrInit, tableSection);
      return bundle(newGrid, details[0].row, details[0].column);
    };
    ...
    const opEraseColumns = (grid, extractDetail, _comparator, _genWrappers) => {
      ...
      const maxColuserindex = newGrid.length > 0 ? newGrid[0].cells.length - 1 : 0;
      return bundle(newGrid, columns[0].row, Math.min(columns[0].column, maxColuserindex));
    };
    const opEraseRows = (grid, details, _comparator, _genWrappers) => {
      const rows = uniqueRows(details);
      const newGrid = deleteRowsAt(grid, rows[0].row, rows[rows.length - 1].row);
      const maxRowuserindex = newGrid.length > 0 ? newGrid.length - 1 : 0;
      return bundle(newGrid, Math.min(details[0].row, maxRowuserindex), details[0].column);
    };
    ...
    const opPasteColsBefore = (grid, pasteDetails, comparator, _genWrappers) => {
      const rows = extractGridDetails(grid).rows;
      const userindex = pasteDetails.cells[0].column;
      const context = rows[pasteDetails.cells[0].row];
      const gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
      const mergedGrid = insertCols(userindex, grid, gridB, pasteDetails.generators, comparator);
      return bundle(mergedGrid, pasteDetails.cells[0].row, pasteDetails.cells[0].column);
    };
    const opPasteColsAfter = (grid, pasteDetails, comparator, _genWrappers) => {
      const rows = extractGridDetails(grid).rows;
      const userindex = pasteDetails.cells[pasteDetails.cells.length - 1].column + pasteDetails.cells[pasteDetails.cells.length - 1].colspan;
      const context = rows[pasteDetails.cells[0].row];
      const gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
      const mergedGrid = insertCols(userindex, grid, gridB, pasteDetails.generators, comparator);
      return bundle(mergedGrid, pasteDetails.cells[0].row, pasteDetails.cells[0].column);
    ...
    const opPasteRowsBefore = (grid, pasteDetails, comparator, _genWrappers) => {
      const rows = extractGridDetails(grid).rows;
      const userindex = pasteDetails.cells[0].row;
      const context = rows[userindex];
      const gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
      const mergedGrid = insertRows(userindex, grid, gridB, pasteDetails.generators, comparator);
      return bundle(mergedGrid, pasteDetails.cells[0].row, pasteDetails.cells[0].column);
    };
    const opPasteRowsAfter = (grid, pasteDetails, comparator, _genWrappers) => {
      const rows = extractGridDetails(grid).rows;
      const userindex = pasteDetails.cells[pasteDetails.cells.length - 1].row + pasteDetails.cells[pasteDetails.cells.length - 1].rowspan;
      const context = rows[pasteDetails.cells[0].row];
      const gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
      const mergedGrid = insertRows(userindex, grid, gridB, pasteDetails.generators, comparator);
      return bundle(mergedGrid, pasteDetails.cells[0].row, pasteDetails.cells[0].column);
    ...
    const redistributeToColumns = (newWidths, columns, unit) => {
      each$2(columns, (column, userindex) => {
        const width = sum([newWidths[userindex]], minWidth());
    ...
    const createRow = (columns, rowHeaders, columnHeaders, rowuserindex) => {
      const tr = SugarElement.fromTag('tr');
      for (let j = 0; j < columns; j++) {
        const td = rowuserindex < rowHeaders || j < columnHeaders ? tableHeaderCell() : tableCell();
        if (j < columnHeaders) {
          set$2(td, 'scope', 'row');
        }
        if (rowuserindex < rowHeaders) {
          set$2(td, 'scope', 'col');
        }
        append$1(td, SugarElement.fromTag('br'));
        append$1(tr, td);
    ...
    const isPercentage = width => isString(width) && width.userindexOf('%') !== -1;
    ...
    const inParent = (parent, children, element, userindex) => ({
      parent,
      children,
      element,
      userindex
    });
    const userindexInParent = element => parent(element).bind(parent => {
      const children = children$2(parent);
      return userindexOf(children, element).map(userindex => inParent(parent, children, element, userindex));
    });
    const userindexOf = (elements, element) => finduserindex(elements, curry(eq$1, element));
    ...
    const handleParent = (isRoot, element, offset, direction) => {
      return findBr(element, offset).bind(br => {
        return direction.traverse(br).fold(() => {
          ...
          return userindexInParent(adjacent).map(info => {
            return Situ.on(info.parent, info.userindex);
          });
        });
      });
    };
    ...
    const resizableColumns = (warehouse, isResizable) => {
      const resizableCols = [];
      range$1(warehouse.grid.columns, userindex => {
        const colElmOpt = Warehouse.getColumnAt(warehouse, userindex).map(col => col.element);
        if (colElmOpt.forall(isResizable)) {
          resizableCols.push(userindex);
        }
      });
      return filter$2(resizableCols, coluserindex => {
        const columnCells = Warehouse.filterItems(warehouse, cell => cell.column === coluserindex);
    ...
    const refreshGrid = (warhouse, wire, table, rows, cols) => {
      ...
      const resizableRowPositions = filter$2(rowPositions, (_pos, i) => exists(resizableRowBars, baruserindex => i === baruserindex));
      ...
      const resizableColPositions = filter$2(colPositions, (_pos, i) => exists(resizableColBars, baruserindex => i === baruserindex));
