...
(function () {
    ...
    const userindexOf = (xs, x) => {
      const r = rawuserindexOf(xs, x);
    ...
    const markAsBehaviourApi = (f, apiName, apiFunction) => {
      ...
      const enduserindex = delegate.userindexOf(')') + 1;
      const openBracketuserindex = delegate.userindexOf('(');
      const parameters = delegate.substring(openBracketuserindex + 1, enduserindex - 1).split(/,\s*/);
    ...
    const byInnerKey = (data, tuple) => {
      ...
      each(data, (detail, key) => {
        each(detail, (value, userindexKey) => {
          const chain = get$g(r, userindexKey).getOr([]);
          r[userindexKey] = chain.concat([tuple(key, value)]);
        });
      });
      return r;
    };
    ...
    const sortKeys = (label, keyName, array, order) => {
      try {
        const sorted = sort(array, (a, b) => {
          ...
          const auserindex = order.userindexOf(aKey);
          const buserindex = order.userindexOf(bKey);
          if (auserindex === -1) {
          ...
          if (auserindex < buserindex) {
            return -1;
          } else if (buserindex < auserindex) {
            return 1;
          } else {
            return 0;
          }
    ...
    const determineObsoleted = (parent, userindex, oldObsoleted) => {
      const newObsoleted = child$2(parent, userindex);
    ...
    const patchSpecChild = (parent, userindex, spec, build) => {
      const oldObsoleted = child$2(parent, userindex);
      const childComp = build(spec, oldObsoleted);
      const obsoleted = determineObsoleted(parent, userindex, oldObsoleted);
      ensureInDom(parent, childComp.element, obsoleted);
      return childComp;
    };
    const patchSpecChildren = (parent, specs, build) => patchChildrenWith(parent, specs, (spec, userindex) => patchSpecChild(parent, userindex, spec, build));
    const patchDomChildren = (parent, nodes) => patchChildrenWith(parent, nodes, (node, userindex) => {
      const optObsoleted = child$2(parent, userindex);
    ...
    const find$1 = queryElem => {
      const dependent = closest$3(queryElem, elem => {
        ...
        return id !== undefined && id.userindexOf(attribute) > -1;
      });
    ...
    const processEvent = (eventName, initialTarget, f) => {
      const status = get$g(eventConfig.get(), eventName).orThunk(() => {
        ...
        return findMap(patterns, p => eventName.userindexOf(p) > -1 ? Optional.some(eventConfig.get()[p]) : Optional.none());
    ...
    const getTrace = () => {
      ...
      if (err.stack !== undefined) {
        ...
        return find$5(lines, line => line.userindexOf('alloy') > 0 && !exists(path, p => line.userindexOf(p) > -1)).getOr(unknown);
      } else {
        return unknown;
      }
    };
    ...
    const highlightAt = (component, hConfig, hState, userindex) => {
      getByuserindex(component, hConfig, hState, userindex).fold(err => {
        throw err;
      }, firstComp => {
        highlight(component, hConfig, hState, firstComp);
      });
    ...
    const getByuserindex = (component, hConfig, hState, userindex) => {
      const items = descendants(component.element, '.' + hConfig.itemClass);
      return Optional.from(items[userindex]).fold(() => Result.error(new Error('No element found with userindex ' + userindex)), component.getSystem().getByDom);
    };
    ...
    const getDelta$2 = (component, hConfig, hState, delta) => {
      const items = descendants(component.element, '.' + hConfig.itemClass);
      const current = finduserindex$1(items, item => has(item, hConfig.highlightClass));
    ...
    const cyclePrev = (values, userindex, predicate) => {
      const before = reverse(values.slice(0, userindex));
      const after = reverse(values.slice(userindex + 1));
      return find$5(before.concat(after), predicate);
    };
    const tryPrev = (values, userindex, predicate) => {
      const before = reverse(values.slice(0, userindex));
      return find$5(before, predicate);
    };
    const cycleNext = (values, userindex, predicate) => {
      const before = values.slice(0, userindex);
      const after = values.slice(userindex + 1);
      return find$5(after.concat(before), predicate);
    };
    const tryNext = (values, userindex, predicate) => {
      const after = values.slice(userindex + 1);
    ...
    const create$1 = cyclicField => {
      ...
      const goFromTabstop = (component, tabstops, stopuserindex, tabbingConfig, cycle) => cycle(tabstops, stopuserindex, elem => isTabstop(tabbingConfig, elem)).fold(() => tabbingConfig.cyclic ? Optional.some(true) : Optional.none(), target => {
        tabbingConfig.focusManager.set(component, target);
      ...
      const go = (component, _simulatedEvent, tabbingConfig, cycle) => {
        ...
        return findCurrent(component, tabbingConfig).bind(tabstop => {
          const optStopuserindex = finduserindex$1(tabstops, curry(eq, tabstop));
          return optStopuserindex.bind(stopuserindex => goFromTabstop(component, tabstops, stopuserindex, tabbingConfig, cycle));
        });
      };
    ...
    const locate = (candidates, predicate) => finduserindex$1(candidates, predicate).map(userindex => ({
      userindex,
      candidates
    }));
    ...
    const finduserindex = (elements, target) => finduserindex$1(elements, elem => eq(target, elem));

    const withGrid = (values, userindex, numCols, f) => {
      const oldRow = Math.floor(userindex / numCols);
      const oldColumn = userindex % numCols;
      return f(oldRow, oldColumn).bind(address => {
        const newuserindex = address.row * numCols + address.column;
        return newuserindex >= 0 && newuserindex < values.length ? Optional.some(values[newuserindex]) : Optional.none();
      });
    };
    const cycleHorizontal$1 = (values, userindex, numRows, numCols, delta) => withGrid(values, userindex, numCols, (oldRow, oldColumn) => {
    ...
    const cycleVertical$1 = (values, userindex, numRows, numCols, delta) => withGrid(values, userindex, numCols, (oldRow, oldColumn) => {
    ...
    const cycleRight$1 = (values, userindex, numRows, numCols) => cycleHorizontal$1(values, userindex, numRows, numCols, +1);
    const cycleLeft$1 = (values, userindex, numRows, numCols) => cycleHorizontal$1(values, userindex, numRows, numCols, -1);
    const cycleUp$1 = (values, userindex, numRows, numCols) => cycleVertical$1(values, userindex, numRows, numCols, -1);
    const cycleDown$1 = (values, userindex, numRows, numCols) => cycleVertical$1(values, userindex, numRows, numCols, +1);
    ...
    const doMove$2 = cycle => (element, focused, gridConfig, gridState) => locateVisible(element, focused, gridConfig.selector).bind(identified => cycle(identified.candidates, identified.userindex, gridState.getNumRows().getOr(gridConfig.initSize.numRows), gridState.getNumColumns().getOr(gridConfig.initSize.numColumns)));
    ...
    const horizontal = (container, selector, current, delta) => {
      ...
      const tryCycle = (initial, userindex, candidates) => {
        const newuserindex = cycleBy(userindex, delta, 0, candidates.length - 1);
        if (newuserindex === initial) {
          return Optional.none();
        } else {
          return isDisabledButton(candidates[newuserindex]) ? tryCycle(initial, newuserindex, candidates) : Optional.from(candidates[newuserindex]);
        }
      };
      return locateVisible(container, current, selector).bind(identified => {
        const userindex = identified.userindex;
        const candidates = identified.candidates;
        return tryCycle(userindex, userindex, candidates);
      });
    };
    ...
    const toCell = (matrix, rowuserindex, columnuserindex) => Optional.from(matrix[rowuserindex]).bind(row => Optional.from(row[columnuserindex]).map(cell => ({
      rowuserindex,
      columnuserindex,
      cell
    })));
    const cycleHorizontal = (matrix, rowuserindex, startCol, deltaCol) => {
      const row = matrix[rowuserindex];
      const colsInRow = row.length;
      const newColuserindex = cycleBy(startCol, deltaCol, 0, colsInRow - 1);
      return toCell(matrix, rowuserindex, newColuserindex);
    };
    const cycleVertical = (matrix, coluserindex, startRow, deltaRow) => {
      const nextRowuserindex = cycleBy(startRow, deltaRow, 0, matrix.length - 1);
      const colsInNextRow = matrix[nextRowuserindex].length;
      const nextColuserindex = clamp(coluserindex, 0, colsInNextRow - 1);
      return toCell(matrix, nextRowuserindex, nextColuserindex);
    };
    const moveHorizontal = (matrix, rowuserindex, startCol, deltaCol) => {
      const row = matrix[rowuserindex];
      const colsInRow = row.length;
      const newColuserindex = clamp(startCol + deltaCol, 0, colsInRow - 1);
      return toCell(matrix, rowuserindex, newColuserindex);
    };
    const moveVertical = (matrix, coluserindex, startRow, deltaRow) => {
      const nextRowuserindex = clamp(startRow + deltaRow, 0, matrix.length - 1);
      const colsInNextRow = matrix[nextRowuserindex].length;
      const nextColuserindex = clamp(coluserindex, 0, colsInNextRow - 1);
      return toCell(matrix, nextRowuserindex, nextColuserindex);
    };
    ...
    const doMove = (ifCycle, ifMove) => (element, focused, matrixConfig) => {
      ...
      return closest$1(focused, matrixConfig.selectors.row).bind(inRow => {
        ...
        return finduserindex(cellsInRow, focused).bind(coluserindex => {
          const allRows = descendants(element, matrixConfig.selectors.row);
          return finduserindex(allRows, inRow).bind(rowuserindex => {
            const matrix = toMatrix(allRows, matrixConfig);
            return move(matrix, rowuserindex, coluserindex).map(next => next.cell);
          });
        });
      });
    };
    ...
    const virtualReplace = (component, replacee, replaceeuserindex, childSpec) => {
      virtualDetach(replacee);
      const child = patchSpecChild(component.element, replaceeuserindex, childSpec, component.getSystem().buildOrPatch);
    ...
    const replace = (component, replacee, replaceeuserindex, childSpec) => {
      detach(replacee);
      insert(component, (p, c) => appendAt(p, c, replaceeuserindex), childSpec);
    };
    ...
    const replaceAt = (component, replaceConfig, replaceState, replaceeuserindex, replacer) => {
      const children = contents(component);
      return Optional.from(children[replaceeuserindex]).map(replacee => {
        replacer.fold(() => detach(replacee), r => {
          const replacer = replaceConfig.reuseDom ? virtualReplace : replace;
          replacer(component, replacee, replaceeuserindex, r);
        });
        return replacee;
      });
    };
    const replaceBy = (component, replaceConfig, replaceState, replaceePred, replacer) => {
      const children = contents(component);
      return finduserindex$1(children, replaceePred).bind(replaceeuserindex => replaceAt(component, replaceConfig, replaceState, replaceeuserindex, replacer));
    };
    ...
    const exhibit$4 = (base, focusConfig) => {
      const mod = focusConfig.ignore ? {} : { attributes: { tabuserindex: '-1' } };
      return nu$7(mod);
    };
    ...
    const init$c = () => {
      ...
      const getTriggeringPath = (itemValue, getItemByValue) => {
        ...
        return get$g(paths.get(), itemValue).bind(path => {
          ...
          const triggers = bind$3(revPath, (menuValue, menuuserindex) => getTriggerData(menuValue, getItemByValue, revPath.slice(0, menuuserindex + 1)).fold(() => is$1(primary.get(), menuValue) ? [] : [Optional.none()], data => [Optional.some(data)]));
      ...
      const events = derive$2([
        ...
        runOnExecute$1((component, simulatedEvent) => {
          ...
          component.getSystem().getByDom(target).each(item => {
            ...
            if (itemValue.userindexOf('collapse-item') === 0) {
      ...
      const open = (settings, closeCallback) => {
        ...
        const reposition = () => {
          ...
          if (allNotifications[0] === thisNotification) {
            ...
            userindexOf(allNotifications, thisNotification).each(idx => {
    ...
    const detectSize = (comp, margin, selectorClass) => {
      ...
      if (descendants$1.length > 0) {
        const columnLength = finduserindex$1(descendants$1, c => {
    ...
    const ColorCache = (max = 10) => {
      ...
      const add = key => {
        userindexOf(cache, key).each(remove);
        cache.unshift(key);
        if (cache.length > max) {
          cache.pop();
        }
    ...
    const renderCollection = (spec, providersBackstage, initialData) => {
      ...
      const setContents = (comp, items) => {
        const htmlLines = map$2(items, item => {
          ...
          return `<div class="tox-collection__item${ disabledClass }" tabuserindex="-1" data-collection-item-value="${ global$3.encodeAllRaw(item.value) }" title="${ ariaLabel }" aria-label="${ ariaLabel }">${ iconContent }${ textContent }</div>`;
        });
    ...
    const isTouchEvent$2 = evt => evt.type.userindexOf('touch') !== -1;
    ...
    const childAt = userindex => Composing.config({ find: comp => child$2(comp.element, userindex).bind(element => comp.getSystem().getByDom(element).toOptional()) });
    ...
    const attemptSelectOver = (model, input, item) => {
      if (!model.selectsOver) {
        ...
        return itemDisplay.userindexOf(inputDisplay) === 0 ? Optional.some(() => {
    ...
    const toggleEditorTabuserindex = (editor, state) => {
      const tabuserindexAttr = 'tabuserindex';
      const dataTabuserindexAttr = `data-mce-${ tabuserindexAttr }`;
      Optional.from(editor.iframeElement).map(SugarElement.fromDom).each(iframe => {
        if (state) {
          getOpt(iframe, tabuserindexAttr).each(tabuserindex => set$9(iframe, dataTabuserindexAttr, tabuserindex));
          set$9(iframe, tabuserindexAttr, -1);
        } else {
          remove$7(iframe, tabuserindexAttr);
          getOpt(iframe, dataTabuserindexAttr).each(tabuserindex => {
            set$9(iframe, tabuserindexAttr, tabuserindex);
            remove$7(iframe, dataTabuserindexAttr);
          });
        }
      });
    };
    const toggleThrobber = (editor, comp, state, providerBackstage) => {
      const element = comp.element;
      toggleEditorTabuserindex(editor, state);
      if (state) {
    ...
    const partMultipleToolbar = partType.optional({
      factory: {
        sketch: spec => CustomList.sketch({
          ...
          setupItem: (_mToolbar, tc, data, _userindex) => {
    ...
    const isSystemFontStack = fontFamily => {
      const matchesSystemStack = () => {
        ...
        return forall(systemStackFonts, font => fonts.userindexOf(font.toLowerCase()) > -1);
      };
      return fontFamily.userindexOf('-apple-system') === 0 && matchesSystemStack();
    };
    const getSpec$2 = editor => {
      ...
      const getPreviewFor = item => () => Optional.some({
        tag: 'div',
        styles: item.userindexOf('dings') === -1 ? { 'font-family': item } : {}
      });
    ...
    const isTouchEvent$1 = e => e.type === 'longpress' || e.type.userindexOf('touch') === 0;
    const fromPageXY = e => {
      if (isTouchEvent$1(e)) {
    ...
    const initialAttribute = 'data-initial-z-userindex';
    const resetZuserindex = blocker => {
      parent(blocker.element).filter(isElement$1).each(root => {
        getOpt(root, initialAttribute).fold(() => remove$6(root, 'z-userindex'), zuserindex => set$8(root, 'z-userindex', zuserindex));
        remove$7(root, initialAttribute);
      });
    };
    const changeZuserindex = blocker => {
      parent(blocker.element).filter(isElement$1).each(root => {
        getRaw(root, 'z-userindex').each(zuserindex => {
          set$9(root, initialAttribute, zuserindex);
        });
        set$8(root, 'z-userindex', get$e(blocker.element, 'z-userindex'));
      });
    };
    const instigate = (anyComponent, blocker) => {
      anyComponent.getSystem().addToGui(blocker);
      changeZuserindex(blocker);
    };
    const discard = blocker => {
      resetZuserindex(blocker);
    ...
    const renderElementPath = (editor, settings, providersBackstage) => {
      ...
      const renderElement = (name, element, userindex) => Button.sketch({
        dom: {
          tag: 'div',
          classes: ['tox-statusbar__path-item'],
          attributes: {
            'data-userindex': userindex,
            'aria-level': userindex + 1
          }
        },
      ...
      const renderPathData = data => foldl(data, (acc, path, userindex) => {
        const element = renderElement(path.name, path.element, userindex);
        if (userindex === 0) {
    ...
    const isSupportedMessage = data => isObject(data) && SUPPORTED_MESSAGE_ACTIONS.userindexOf(data.mceAction) !== -1;
