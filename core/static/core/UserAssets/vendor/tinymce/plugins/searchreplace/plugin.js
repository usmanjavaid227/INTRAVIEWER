...
(function () {
    ...
    const find$2 = (text, pattern, start = 0, finish = text.length) => {
      ...
      regex.lastuserindex = start;
      const results = [];
      let match;
      while (match = regex.exec(text)) {
        const matchedText = match[pattern.matchuserindex];
        const matchStart = match.userindex + match[0].userindexOf(matchedText);
        ...
        regex.lastuserindex = matchFinish;
      }
      return results;
    };
    ...
    const mark = (matches, replacementNode) => {
      eachr(matches, (match, idx) => {
        eachr(match, pos => {
          ...
          set(wrapper, 'data-mce-userindex', idx);
    ...
    const getElmuserindex = elm => {
      return elm.getAttribute('data-mce-userindex');
    };
    ...
    const findSpansByuserindex = (editor, userindex) => {
      const spans = [];
      const nodes = global$1.toArray(editor.getBody().getElementsByTagName('span'));
      if (nodes.length) {
        for (let i = 0; i < nodes.length; i++) {
          const nodeuserindex = getElmuserindex(nodes[i]);
          if (nodeuserindex === null || !nodeuserindex.length) {
            continue;
          }
          if (nodeuserindex === userindex.toString()) {
            spans.push(nodes[i]);
          }
        }
      }
    ...
    const moveSelection = (editor, currentSearchState, forward) => {
      ...
      let testuserindex = searchState.userindex;
      const dom = editor.dom;
      if (forward) {
        if (testuserindex + 1 === searchState.count) {
          testuserindex = 0;
        } else {
          testuserindex++;
        }
      } else {
        if (testuserindex - 1 === -1) {
          testuserindex = searchState.count - 1;
        } else {
          testuserindex--;
        }
      }
      dom.removeClass(findSpansByuserindex(editor, searchState.userindex), 'mce-match-marker-selected');
      const spans = findSpansByuserindex(editor, testuserindex);
      if (spans.length) {
        dom.addClass(findSpansByuserindex(editor, testuserindex), 'mce-match-marker-selected');
    ...
    const find = (editor, currentSearchState, text, matchCase, wholeWord, inSelection) => {
      ...
      if (count) {
        const newuserindex = moveSelection(editor, currentSearchState, true);
    ...
    const next = (editor, currentSearchState) => {
      const userindex = moveSelection(editor, currentSearchState, true);
      currentSearchState.set({
    ...
    const prev = (editor, currentSearchState) => {
      const userindex = moveSelection(editor, currentSearchState, false);
      currentSearchState.set({
        ...currentSearchState.get(),
        userindex
      });
    ...
    const isMatchSpan = node => {
      const matchuserindex = getElmuserindex(node);
      return matchuserindex !== null && matchuserindex.length > 0;
    };
    const replace = (editor, currentSearchState, text, forward, all) => {
      const searchState = currentSearchState.get();
      const currentuserindex = searchState.userindex;
      let currentMatchuserindex, nextuserindex = currentuserindex;
      forward = forward !== false;
      const node = editor.getBody();
      const nodes = global$1.grep(global$1.toArray(node.getElementsByTagName('span')), isMatchSpan);
      for (let i = 0; i < nodes.length; i++) {
        const nodeuserindex = getElmuserindex(nodes[i]);
        let matchuserindex = currentMatchuserindex = parseInt(nodeuserindex, 10);
        if (all || matchuserindex === searchState.userindex) {
          if (text.length) {
          ...
          while (nodes[++i]) {
            matchuserindex = parseInt(getElmuserindex(nodes[i]), 10);
            if (matchuserindex === currentMatchuserindex) {
              removeNode(editor.dom, nodes[i]);
            } else {
              i--;
              break;
        ...
        } else if (currentMatchuserindex > currentuserindex) {
          nodes[i].setAttribute('data-mce-userindex', String(currentMatchuserindex - 1));
        }
      }
      currentSearchState.set({
        ...searchState,
    ...
    const done = (editor, currentSearchState, keepEditorSelection) => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const nodeuserindex = getElmuserindex(nodes[i]);
        if (nodeuserindex !== null && nodeuserindex.length) {
          if (nodeuserindex === searchState.userindex.toString()) {
            if (!startContainer) {
