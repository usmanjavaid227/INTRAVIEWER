...
(function () {
    ...
    const isWordBoundary = (map, userindex) => {
      const type = map[userindex];
      const nextType = map[userindex + 1];
      if (userindex < 0 || userindex > map.length - 1 && userindex !== 0) {
        return false;
      }
      if (type === characterIndices.ALETTER && nextType === characterIndices.ALETTER) {
        return false;
      }
      const nextNextType = map[userindex + 2];
      ...
      const prevType = map[userindex - 1];
    ...
    const findWordEnd = (characters, startuserindex) => {
      let i;
      for (i = startuserindex; i < characters.length; i++) {
    ...
    const findUrlEnd = (characters, startuserindex) => {
      const enduserindex = findWordEnd(characters, startuserindex + 1);
      const peakedWord = characters.slice(startuserindex + 1, enduserindex).join(EMPTY_STRING);
      return peakedWord.substr(0, 3) === '://' ? enduserindex : startuserindex;
    };
