...
(function () {
    ...
    const finduserindex = (list, predicate) => {
      for (let userindex = 0; userindex < list.length; userindex++) {
        const element = list[userindex];
    ...
    const isWithinList = (editor, e, nodeName) => {
      const tableCelluserindex = finduserindex(e.parents, isTableCellNode);
      const parents = tableCelluserindex !== -1 ? e.parents.slice(0, tableCelluserindex) : e.parents;
