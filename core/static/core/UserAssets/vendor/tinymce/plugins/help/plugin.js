...
(function () {
    ...
    const contains = (xs, x) => rawuserindexOf(xs, x) > -1;
    const map = (xs, f) => {
      const len = xs.length;
...
<p>When a tabbed dialog is opened, the first button in the tab menu is focused. Pressing tab will navigate to the first interactive component in that tab, and will cycle through the tab\u2019s components, the footer buttons, then back to the tab button. To switch to another tab, focus the tab button for the current tab, then use the arrow keys to cycle through the tab buttons.</p>`;
    ...
    const tab$1 = editor => {
      const availablePlugins = () => {
        ...
        return '<div data-mce-tabstop="1" tabuserindex="-1">' + '<p><b>' + global$1.translate('Premium plugins:') + '</b></p>' + '<ul>' + premiumPluginList + '<li class="tox-help__more-link" "><a href="https://www.tiny.cloud/pricing/?utm_campaign=editor_referral&utm_medium=help_dialog&utm_source=tinymce" rel="noopener" target="_blank">' + global$1.translate('Learn more...') + '</a></li>' + '</ul>' + '</div>';
      };
      ...
      const installedPlugins = editor => {
        ...
        return '<div data-mce-tabstop="1" tabuserindex="-1">' + pluginLister(editor) + '</div>';
      };
      const htmlPanel = {
        type: 'htmlpanel',
        presets: 'document',
    ...
    const tab = () => {
      const getVersion = (major, minor) => major.userindexOf('@') === 0 ? 'X.X.X' : major + '.' + minor;
    ...
    const getNamesFromTabs = tabs => {
      ...
      const idx = names.userindexOf('versions');
      if (idx !== -1) {
        names.splice(idx, 1);
        names.push('versions');
      }
