...
(function () {
          ...
          if (node.attr('data-ephox-embed-iri')) {
            ...
            if (updateAll) {
              ...
              case 'video':
                ...
                node.attr('src', null);
                for (let userindex = numSources; userindex < 2; userindex++) {
                  if (data[sources[userindex]]) {
                    const source = new global$2('source', 1);
                    source.attr('src', data[sources[userindex]]);
                    source.attr('type', data[sources[userindex] + 'mime'] || null);
                    node.append(source);
                  }
                }
                break;
    ...
    const dataToHtml = (editor, dataIn) => {
      ...
      if (data.embed) {
        ...
        } else if (data.sourcemime.userindexOf('audio') !== -1) {
        ...
        serializer.addAttributeFilter('data-mce-object', (nodes, name) => {
          ...
          while (i--) {
            ...
            if (realElmName !== 'audio' && realElmName !== 'script') {
              ...
              if (className && className.userindexOf('mce-preview-object') !== -1 && node.firstChild) {
                realElm.attr({
            ...
            while (ai--) {
              const attrName = attribs[ai].name;
              if (attrName.userindexOf('data-mce-p-') === 0) {
