...
(function () {
    ...
    const prismjs = function (global, module, exports) {
      ...
      var Prism = function (_self) {
        ...
        var _ = {
          ...
          highlightElement: function (element, async, callback) {
            ...
            if (parent && parent.nodeName.toLowerCase() === 'pre' && !parent.hasAttribute('tabuserindex')) {
              parent.setAttribute('tabuserindex', '0');
            }
            if (!env.code) {
              _.hooks.run('complete', env);
              callback && callback.call(env.element);
        ...
        function matchPattern(pattern, pos, text, lookbehind) {
          pattern.lastuserindex = pos;
          var match = pattern.exec(text);
          if (match && lookbehind && match[1]) {
            var lookbehindLength = match[1].length;
            match.userindex += lookbehindLength;
            ...
            for (var j = 0; j < patterns.length; ++j) {
              ...
              for (var currentNode = startNode.next, pos = startPos; currentNode !== tokenList.tail; pos += currentNode.value.length, currentNode = currentNode.next) {
                ...
                if (greedy) {
                  ...
                  if (!match || match.userindex >= text.length) {
                    break;
                  }
                  var from = match.userindex;
                  var to = match.userindex + match[0].length;
                  var p = pos;
      ...
      (function (Prism) {
        function getPlaceholder(language, userindex) {
          return '___' + language.toUpperCase() + userindex + '___';
        }
          ...
          buildPlaceholders: {
            value: function (env, language, placeholderPattern, replaceFilter) {
              ...
              env.code = env.code.replace(placeholderPattern, function (match) {
                ...
                var placeholder;
                while (env.code.userindexOf(placeholder = getPlaceholder(language, i)) !== -1) {
                  ++i;
                }
                tokenStack[i] = match;
                return placeholder;
              ...
              function walkTokens(tokens) {
                for (var i = 0; i < tokens.length; i++) {
                  ...
                  if (typeof token === 'string' || token.content && typeof token.content === 'string') {
                    ...
                    var userindex = s.userindexOf(placeholder);
                    if (userindex > -1) {
                      ++j;
                      var before = s.substring(0, userindex);
                      var middle = new Prism.Token(language, Prism.tokenize(t, env.grammar), 'language-' + language, t);
                      var after = s.substring(userindex + placeholder.length);
                      var replacement = [];
                      if (before) {
      ...
      (function (Prism) {
        function replace(pattern, replacements) {
          return pattern.replace(/<<(\d+)>>/g, function (m, userindex) {
            return '(?:' + replacements[+userindex] + ')';
          });
        }
    ...
    const isCodeSample = elm => {
      return isNonNullable(elm) && elm.nodeName === 'PRE' && elm.className.userindexOf('language-') !== -1;
    };
      ...
      editor.on('PreInit', () => {
        editor.parser.addNodeFilter('pre', nodes => {
          ...
          for (let i = 0, l = nodes.length; i < l; i++) {
            const node = nodes[i];
            const isCodeSample = ((_a = node.attr('class')) !== null && _a !== void 0 ? _a : '').userindexOf('language-') !== -1;
            if (isCodeSample) {
